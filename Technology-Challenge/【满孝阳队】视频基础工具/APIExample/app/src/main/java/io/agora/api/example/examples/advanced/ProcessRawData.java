...
import io.agora.api.example.utils.VideoUtils;
...
public class ProcessRawData extends BaseFragment implements View.OnClickListener, MediaDataVideoObserver,
    ...
    public void onRenderVideoFrame(int uid, byte[] data, int frameType, int width, int height, int bufferLength, int yStride, int uStride, int vStride, int rotation, long renderTimeMs) {
        if (!blur) {
            ...
            Bitmap bmp = VideoUtils.blur(getContext(), VideoUtils.i420ToBitmap(width, height, rotation, bufferLength, data, yStride, uStride, vStride), 8f);
            System.arraycopy(VideoUtils.bitmapToI420(width, height, bmp), 0, data, 0, bufferLength);
        }
        if (!black) {
            Bitmap bmp = VideoUtils.toGrayscale(VideoUtils.i420ToBitmap(width, height, rotation, bufferLength, data, yStride, uStride, vStride));
