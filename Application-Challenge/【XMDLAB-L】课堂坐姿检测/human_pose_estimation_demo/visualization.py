...
def show_poses(img, poses, scores, pose_score_threshold=0.5, point_score_threshold=0.5, skeleton=None, draw_ellipses=False):
    ...
    for i,(pose, pose_score) in enumerate(zip(poses, scores)):
        ...
        # 功能实现部分
        len1 = abs(points[5][0] - points[6][0])
        len2 = abs(points[5][1] - points[6][1])
        len = (len1 * len1 + len2 * len2) ** 0.5

        # Draw joints.
        for j, (p, v) in enumerate(zip(points, points_scores)):
            if v > point_score_threshold:
                if j <= 6:
                    my_poses.append(p)
                #cv2.circle(img, tuple(p), 1, colors[i], 2)#骨骼点信息
                text=str(j)
                #cv2.putText(img, text, tuple(p), cv2.FONT_HERSHEY_COMPLEX, 1.5, (0, 0, 200), 2)
                #print(j, points[j][0], points[j][1])
