package com.kangaroo.openlive.ui;
...
public class ResolutionAdapter extends RecyclerView.Adapter {
    ...
    private void initData(Context context) {
        int size = com.kangaroo.openlive.Constants.VIDEO_DIMENSIONS.length;
        String[] labels = context.getResources().
                getStringArray(R.array.string_array_resolutions);

        for (int i = 0; i < size; i++) {
            ResolutionItem item = new ResolutionItem(labels[i], com.kangaroo.openlive.Constants.VIDEO_DIMENSIONS[i]);
            mItems.add(item);
        }
    }
