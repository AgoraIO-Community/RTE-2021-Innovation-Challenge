...
@interface OnlineSearchVC  ()<UISearchBarDelegate,UITableViewDelegate,UITableViewDataSource>
{
...
-(void)createButtonViewAndButton
{
    ...
    typeButton.frame = CGRectMake(WIDTH - 50, 27, 48, 30);
...
//        _buttonListArray = [NSMutableArray arrayWithObjects:
//        @"电子图书",
//                            @"有声图书",
//                            @"国学经典",
//                            @"书法分馆",
...
//                            @"名师绘画",
//                            nil];
//        _typeIdArray = [NSMutableArray arrayWithObjects:
//        @"22",
//                        @"55",
//                        @"88",
//                        @"33",
...
{
    NSArray *classArray = [NSArray arrayWithObjects:
                           @"22",
                           @"55",
                           @"88",
                           @"33",
...
-(void)searchClick:(UIButton *)button
{
    ...
    _typeView.hidden = YES;
    if (_loacAndOnlionView == nil) {
        _loacAndOnlionView = [[UIView alloc] initWithFrame:CGRectMake(0, 64, WIDTH, 100)];
        UIButton *onliButton = [UIButton buttonWithType:UIButtonTypeCustom];
        
        [onliButton setTitle:@"远程数据" forState: UIControlStateNormal];
        
        onliButton.frame = CGRectMake(0, 0, WIDTH, 49);
        
        [onliButton addTarget:self action:@selector(onliClick:) forControlEvents:UIControlEventTouchUpInside];
        
        [_loacAndOnlionView addSubview:onliButton];
        
        UILabel *li = [[UILabel alloc] initWithFrame:CGRectMake(0, 49, WIDTH, 1)];
        
        li.backgroundColor = [UIColor whiteColor];
        
        [_loacAndOnlionView addSubview:li];
        
        UIButton *locButton = [UIButton buttonWithType:UIButtonTypeCustom];
        
        [locButton setTitle:@"本地数据" forState: UIControlStateNormal];
        
        locButton.frame = CGRectMake(0, 50, WIDTH, 50);
        
        [locButton addTarget:self action:@selector(loaClick:) forControlEvents:UIControlEventTouchUpInside];
        
        [_loacAndOnlionView addSubview:locButton];
        
        _loacAndOnlionView.backgroundColor = [UIColor lightGrayColor];
        [self.view addSubview:_loacAndOnlionView];
        
        
        
    }else{
        _loacAndOnlionView.hidden = !_loacAndOnlionView.hidden;
    }
...
-(void)serchRerultForSverse:(NSString *)str WithType:(NSString *)type
{
    ...
    NSString *port = [NSString stringWithFormat:@"http://resource.cncgroup.net:8015/api/books/Search/%@/%@?pageIndex=0&pageSize=100",type,str];
