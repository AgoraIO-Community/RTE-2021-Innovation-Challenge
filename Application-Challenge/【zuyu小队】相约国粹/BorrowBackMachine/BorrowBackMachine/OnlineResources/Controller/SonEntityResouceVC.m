...
@interface SonEntityResouceVC ()<UINavigationControllerDelegate,NavgationViewDelegate,QRCodeReaderDelegate,UICollectionViewDelegate,UICollectionViewDataSource,CollctionHeaderButtonsViewDelegate,FirstNavViewDelegate>
{
...
@implementation SonEntityResouceVC
...
-(void)requesRecommendData
{
    ...
    [manager POST:PORT(@"InteBook/GetRecommendList") parameters:parameter progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (responses.statusCode == 401) {
            ...
            [ZuyuTokenRefresh tokenRefreshSuccess:^(NSURLSessionDataTask * _Nonnull dataTaskpp, id  _Nullable responseObject) {
                [self requesHotData];
            } failure:^(NSURLSessionDataTask * _Nullable dataTask, NSError * _Nonnull error) {
            }];
