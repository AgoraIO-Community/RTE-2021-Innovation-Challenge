...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden {
    if ([super initWithFrame:frame]) {
        ...
        // 设置引导页上的跳过按钮
        UIButton *skipButton = [[UIButton alloc]initWithFrame:CGRectMake(DDScreenW*0.85, DDScreenW*0.1, 50, 25)];
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if (i == imageNameArray.count-1 && isHidden == NO) {
                ...
                UIButton *startButton = [[UIButton alloc]initWithFrame:CGRectMake(DDScreenW*0.2, DDScreenH*0.7 - 40, DDScreenW*0.6,140)];
//                [startButton setTitle:@"开始体验" forState:UIControlStateNormal];
                ...
                [startButton setBackgroundImage:[UIImage imageNamed:@"logo_boby_zc_first"] forState:UIControlStateNormal];
...
//                startButton.backgroundColor = [UIColor blackColor];
        ...
        // 设置引导页上的页面控制器
//        self.imagePageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(DDScreenW*0.0, DDScreenH*0.94, DDScreenW*1.0, DDScreenH*0.1)];
...
//        self.imagePageControl.currentPageIndicatorTintColor = [UIColor colorWithRed:20.f/255.f green:175.f/255.f blue:255.f/255.f alpha:1];
...
- (void)buttonClick:(UIButton *)button {
    [self.delegate DoEndWelcomePage];
