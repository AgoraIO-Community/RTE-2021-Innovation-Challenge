...
@interface MapForMachineVC ()<MAMapViewDelegate,NavgationViewDelegate>
{
    //大屏大头针
...
@implementation MapForMachineVC
...
-(void)requestData
{
    ...
    [manager POST:PORT(@"Equipment/GetPagedData") parameters:@"{}" progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if ([NSString stringWithFormat:@"%@",[responseObject objectForKey:@"IsDone"]].integerValue) {
            ...
            for (NSDictionary *itme  in dataDic) {
                ...
                pointAnnotation.title = model.EquipmentName;
                pointAnnotation.subtitle = model.EquipmentAddress;
...
-(void)requestEasyData
{
    ...
    [manager POST:PORT(@"Venue/GetAll") parameters:@"{}" progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if ([NSString stringWithFormat:@"%@",[responseObject objectForKey:@"IsDone"]].integerValue) {
            ...
            for (NSDictionary *itme in data) {
                ...
                EasyModel *model = [[EasyModel alloc]  init];
                model.Id = [ZuyuJsonRead jsonRead:itme WithKey:@"Id"];
    ...
    if (_page == 1) {
        ...
        if (con+1) {
            ...
            if ([model.Status integerValue] == 1) {
                ...
                BookselfContentVC *vc = [[BookselfContentVC alloc] init];
                vc.hidesBottomBarWhenPushed = YES;
                vc.navTitle = model.title;
                vc.bookSelfID = model.Id;
                [self.navigationController pushViewController:vc animated:YES];
            }else{
                [ZuyuAlertShow alertShow:@"该设备已停用,请选择其他设备" viewController: self];
            }
        ...
        if (con+1) {
            ...
            EasyModel *model = _easyMachineDataArray[con];
            
            EasyBookselfVC *vc = [[EasyBookselfVC alloc] init];
            vc.hidesBottomBarWhenPushed = YES;
            vc.navTitle = model.VenueName;
            vc.bookSelfID = model.Id;
