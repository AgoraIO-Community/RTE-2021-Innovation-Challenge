...
func (nq *NotificationQuery) QueryReceiver() *UserQuery {
	query := &UserQuery{config: nq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (nq *NotificationQuery) Clone() *NotificationQuery {
	...
	return &NotificationQuery{
		...
		withReceiver: nq.withReceiver.Clone(),
...
func (nq *NotificationQuery) WithReceiver(opts ...func(*UserQuery)) *NotificationQuery {
	query := &UserQuery{config: nq.config}
	for _, opt := range opts {
		opt(query)
	}
	nq.withReceiver = query
	return nq
}
...
//		GroupBy(notification.FieldContent).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Notification.Query().
//		Select(notification.FieldContent).
//		Scan(ctx, &v)
//
...
func (nq *NotificationQuery) sqlAll(ctx context.Context) ([]*Notification, error) {
	...
	if nq.withSender != nil || nq.withReceiver != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := nq.withSender; query != nil {
		...
		for i := range nodes {
			if nodes[i].user_send_notifications == nil {
				continue
			}
			fk := *nodes[i].user_send_notifications
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_send_notifications" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := nq.withReceiver; query != nil {
		ids := make([]string, 0, len(nodes))
		nodeids := make(map[string][]*Notification)
		for i := range nodes {
			if nodes[i].user_received_notifications == nil {
				continue
			}
			fk := *nodes[i].user_received_notifications
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_received_notifications" returned %v`, n.ID)
			}
			for i := range nodes {
