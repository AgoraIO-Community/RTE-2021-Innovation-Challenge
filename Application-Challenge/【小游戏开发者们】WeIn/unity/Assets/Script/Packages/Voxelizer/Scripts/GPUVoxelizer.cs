...
namespace VoxelSystem {
	...
	public class GPUVoxelizer {
        ...
        public static GPUVoxelData Voxelize(ComputeShader voxelizer, Mesh mesh, Bounds bounds, int resolution = 32, bool volume = true, bool pow2 = false)
        {
			...
			voxelizer.SetVector(kUnitKey, new Vector3(unit,unit,unit));
			voxelizer.SetVector(kInvUnitKey, new Vector3(1f/unit, 1f/unit, 1f/unit));
			voxelizer.SetVector(kHalfUnitKey, new Vector3(hunit,hunit,hunit));
			voxelizer.SetInt(kWidthKey, w);
			voxelizer.SetInt(kHeightKey, h);
			voxelizer.SetInt(kDepthKey, d);
			...
			return new GPUVoxelData(voxelBuffer, w, h, d, new Vector3( unit,unit,unit));
        }
