...
public class VideoYoloSurface : MonoBehaviour
    {
   ...
   // bool waitEnd = false;
    ...
    void RepeatCall()
    {
      /*  if (!waitEnd)
            return;
        if (yoloWrapper != null && nativeTexture != null)
                   {
                       var container = yoloWrapper.Detect(nativeTexture.EncodeToJPG());
                       if (detectedCallback != null)
                       {
                           detectedCallback.Invoke(container, defWidth, defHeight);
                       }
                   }*/
    }
    void OnDestroy()
