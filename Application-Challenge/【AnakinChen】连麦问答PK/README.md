...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;常见的连麦场景下，双方彼此只能聊天，聊聊家常，看看彼此。而在短视频有一种比较常见的视频题材————问答类型，其形式就是提出一个系列的问题，然后轮流答题，可参考后面 [相关视频](#相关视频) 部分。 
 
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;将 连麦 & 问答 两者结合起来，即通过连麦来进行问答 PK，参与连麦的玩家以轮流的方式回答系统提出的问题，并将答对的数量作为计分的互动游戏。
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;题目在项目的设计上极方便扩展，可以有更多的多样性和趣味性。题目可以类似是 Demo 演示的知识题，也可以可以增加自定义题目（后期增加，暂时没哈），如设置家庭系列的题目，场景可以是：父子的连麦，题目有妈妈最拿手的是什么菜？爷爷奶奶是什么时候结婚的？爸爸的生日？等等。  


# 目标

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;连麦，让距离不再是问题，使互动无法被阻挡。  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;答题，即增进彼此的了解，又扩展大家的知识。 
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;本工程只实现 iOS 客户端部分，需要填写配置信息。

**为方便填写，新增 JSON配置，直接联系我获取即可，方便评委进行设置。**

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;通过 App 的设置进行配置，或者手动在 ```QHAppContext.m p_readSetting``` 函数最后的注释打开后，直接赋值配置信息。而部分信息相对比较隐私，所以如需要可邮件或者微信联系我获取，如有不便，请谅解。

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;需要填写的信息如下：（注意以下例子的参考值非正确信息，只供参考）
...
| 语音格式转换 | 将 声网 采集的音频数据进行转码 | 对连麦采集的音频数据进行处理，一般是采样率，采用格式进行转换，以符合语音识别要求的格式。 |  
| 讯飞语音识别| 将语音进行文字识别，并返回文字结果 | 需要拼接返回的结果。采取完全匹配来判断答案正确与否。 |   
| 问答游戏 | 游戏成员、题型 和 答题 进程和结果校验 & 计分 | 答题游戏题目、答案、参与人员、分数等数据的管理 和 维护。 |  

#### 问答题型：  

| 题型 | 举例 |  
| --- | --- | --- |  
| 一题多答案（暂不再继续支持）| 说出明朝皇帝：朱元璋、朱棣…… |  
| 一系列多问题（唯一答案）| 中国最耀眼的明星：1、中国导弹之父——钱学森 2、…… |  
