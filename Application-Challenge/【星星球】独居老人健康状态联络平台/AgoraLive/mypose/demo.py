...
#################
        ...
        if judge1 and judge2:
            cv2.putText(img, "stand", (200, 200), cv2.FONT_HERSHEY_COMPLEX, 3.5, (255, 0, 0), 2)
            flag1 = 1
        max_h = 0
        min_h = 10000
        for jj in range(16):
        ...
        if high < int(img.shape[0] / 3):
            cv2.putText(img, "lie", (200, 200), cv2.FONT_HERSHEY_COMPLEX, 3.5, (0, 0, 255), 2)
            flag2 = 1
        ...
        if t>10:
            text = 'danger'
            cv2.putText(img, text, (150,150), cv2.FONT_HERSHEY_COMPLEX, 4.5, (0, 0, 255), 2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint-path', type=str, default='checkpoint_iter_370000.pth',help='path to the checkpoint')#到检查点的路
    parser.add_argument('--height-size', type=int, default=256, help='network input layer height size')#网络输入层高度大小
    parser.add_argument('--video', type=str, default='', help='path to video file or camera id')#视频文件或摄像机id的路径
