...
-(BOOL)convert{
    ...
    if (self.inputFile==NULL) {
        debugLog(@"Source file is not set");
        return NO;
    }
    
    if (self.outputFile==NULL) {
        debugLog(@"Output file is no set");
        return NO;
    }
    ...
    debugLog(@"output format:%@",[self descriptionForAudioFormat:settings.outputFormat]);
    ...
    debugLog(@"Client data format:%@",[self descriptionForAudioFormat:settings.inputPCMFormat]);
...
-(void)validateInput:(ExtAudioConverterSettings*)settigs{
    ...
    if (!valid) {
        debugLog(@"the file format and data format pair is not valid");
        exit(-1);
    }
