...
- (void)_setupViews
{
    ...
    titleLabel.text = @"注 册 账 号";
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.textColor = [UIColor colorWithRed:47/255.0 green:195/255.0 blue:154/255.0 alpha:1.0];
...
//    }];
    ...
    [self.authorizationView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(backView).offset(30);
        make.right.equalTo(backView).offset(-30);
        make.top.equalTo(self.confirmPswdField.mas_bottom).offset(90);
        make.height.equalTo(@55);
    }];
}
