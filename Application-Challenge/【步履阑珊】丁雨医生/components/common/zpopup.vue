<template>
	<view @touchmove="onTouchMove">
		<!-- 遮罩层动画 -->
		<view class="mask" @click="(currentValue = false)" 
		:style="{top:maskTop, bottom:maskBottom, zIndex: zIndex}" v-if="currentValue"></view>
		<!-- 显示信息层 -->
		<view class="popup_box" :class="{'bottom': type == 'bottom' ,'center':type == 'center', top: type == 'top'}" 
		:style="{opacity:opacity,transform:transform,top:popupTop, bottom:popupBottom,zIndex: zIndex + 1}">
			<slot></slot>
		</view>
	</view>
</template>
